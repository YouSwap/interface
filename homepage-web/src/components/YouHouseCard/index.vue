...
<template>
  <div class="you-wrapper">
    ...
    <div class="list-title">
      ...
      <div class="title-item title-item3" style="justify-content: flex-end">{{$t('APY')}}</div>
    </div>
    <div class="line"></div>
    <div class="list">
...
export default {
  ...
  data () {
    return {
      YouAddress: process.env.VUE_APP_YOU_CONTRACT_ADDRESS,
      YouPrice: '',
      sortData: []
    }
  },
  ...
  methods: {
    ...
    sortHandle() {
      ...
      sortData.forEach((item) => {
        item.apy = 0
        if (item.reserveUSD || that.YouAddress === item.lpaddress) {
          if (((item.reserveUSD / item.totalSupply * (item.staketotaldnow / 10**item.decimals)) === 0) || (that.YouPrice * (item.staketotaldnow / 10**item.decimals) === 0)) {
            ...
            if (that.YouAddress === item.lpaddress) {
              item.apy = getDecimalsCoin((item.rewardperblock / 10**6 * that.YouPrice * 10512000 * 100) / ( that.YouPrice * (item.staketotaldnow / 10**item.decimals)), 3)
            } else {
              item.apy = getDecimalsCoin((item.rewardperblock / 10**6 * that.YouPrice * 10512000 * 100) / (item.reserveUSD / item.totalSupply * (item.staketotaldnow / 10**item.decimals)), 3)
            }
          }
        }
      ...
      this.sortData = Object.assign([], sortData)
