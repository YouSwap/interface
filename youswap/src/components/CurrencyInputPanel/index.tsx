...
const CurrencySelect = styled.button<{ selected: boolean }>`
  ...
  cursor: pointer;
  user-select: none;
  border: ${({ selected, theme }) => (selected ? 'none' : `1px solid ${theme.text1}`)};
  padding: 0 0.5rem;

  :focus,
  :hover {
    background-color: ${({ selected, theme }) => (selected ? theme.bg1 : darken(0.05, theme.bg1))};
  }
`
...
//   font-weight: 500;
//   background-color: ${({ selected, theme }) => (selected ? theme.bg1 : theme.bg1)};
//   color: ${({ selected, theme }) => (selected ? theme.text1 : '#06253B')};
//   border-radius: 8px;
//   box-shadow: ${({ selected }) => (selected ? 'none' : '0px 6px 10px rgba(0, 0, 0, 0.075)')};
//   outline: none;
//   cursor: pointer;
//   user-select: none;
//   border: ${({ selected }) => (selected ? 'none' : '0px 6px 10px rgba(0, 0, 0, 0.075)')};;
//   padding: 0 0.5rem;
          ...
          <LabelRow>
            ...
            <RowBetween>
              ...
              <TYPE.body color={theme.text2} fontWeight={500} fontSize={14}>
                {label === 'From' ? t('From') : label === 'To' ? t('To') : t(`${label}`)}
              </TYPE.body>
              {account && (
                // Balance 区域文本样式
                <TYPE.body
