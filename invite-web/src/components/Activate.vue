<template>
  <div class="activate-box" @click="cancelHandle">
    <div class="inner-box" :class="platform == 2 ? 'pc-wrap' : 'h5-wrap'" @click.stop>
      ...
      <div class="tab-box">
        <div :class="tabId == 1 ? 'active' : ''" @click="switchHandle(1)">{{$t('activate')[0]}}</div>
        <div :class="tabId == 2 ? 'active' : ''" @click="switchHandle(2)">{{$t('activate')[1]}}</div>
      </div>
      <template v-if="tabId == 1">
        <div class="single-box">
          <input type="text" @blur="changeHandle" v-model="userAddress" :placeholder="$t('activate')[2]">
        </div>
      </template>
      <template v-if="tabId == 2">
        <div class="batch-box">
          <p>{{$t('activate')[5]}}<a href="../address-demo.xlsx" download="address-demo.xlsx">{{$t('activate')[6]}}</a>{{$t('activate')[7]}}<span>{{$t('activate')[19]}}</span>{{$t('activate')[20]}}
          </p>
          <div class="inner">
            <input type="text" v-model="fileName" readonly>
            <div>
              <input type="file" ref="upload" accept=".xls,.xlsx">
              <a href="javascript:;" :class="lang == 'en' ? 'en-version' : ''">{{$t('activate')[8]}}</a>
            </div>
          </div>
        </div>
      </template>
      <a href="javascript:;" class="bottom" @click="activateHandle">
