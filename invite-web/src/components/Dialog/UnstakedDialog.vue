<template>
  <div class="staked-wrapper"
    ...
    <div class="dialog-wrap"
      ...
      <h5>{{$t('digcards')[9]}} {{list_info.poolname}}
      ...
      <div class="btn-wrap">
        ...
        <button :class="notshowConfim?'confirm-disabled': ''"
                ...
                class="confirm"
                @click="handleConfirm">{{$t('dialog')[5]}}</button>
      </div>
    </div>
  </div>
</template>
...
export default {
    ...
    if (this.list_info.user.length) {
      this.list_info.user[0].poolsinfo.forEach(item => {
        if (this.list_info.id == item.id.split('-')[0]) {
          this.balanceof = ethers.utils.formatUnits(item.stake, this.list_info.decimals)
        }
      })
    }
    this.getMaxAmount()
  ...
  methods: {
    ...
    */
    getMaxAmount () {
      const contractAddress = process.env.VUE_APP_DECIMAL_ADDRESS
      const provider = new ethers.providers.Web3Provider(window.ethereum)
      const contract = new ethers.Contract(contractAddress, abiDecimal, provider)
      const contractWithSigner = contract.connect(provider.getSigner())
      contractWithSigner.withdraw(this.$store.state.account, this.list_info.id).then(res => {
        if (res) {
          this.maxAmount = ethers.utils.formatUnits(res, this.list_info.decimals)
    ...
    async sendAsync () {
      ...
      // if (this.maxActive) {
      //   BigNumbers = ethers.utils.parseUnits(this.amount, decimals).sub(300)
      //   if (BigNumbers < 0) {
      //     BigNumbers = 0
      //   }
      // } else {
      //   BigNumbers = ethers.utils.parseUnits(this.amount, decimals)
      // }

      BigNumbers = ethers.utils.parseUnits(this.amount, decimals)
      if (this.maxAmount && this.maxAmount <= this.amount) {
        BigNumbers = ethers.utils.parseUnits(this.maxAmount, decimals)
      }
